...
class BlackjackGame {
    ...
    static validDeck(deck) {
        if (!Array.isArray(deck)) {
            return false;
        }
        const copy = [...deck];
        copy.sort();
        return copy.length === sortedFullDeck.length && copy.every((card, index) => card === sortedFullDeck[index]);
    }

    setInitialDeck(deck) {
        if (BlackjackGame.validDeck(deck)) {
            this.controlCh.initialInputQueue = deck;
            this.controlCh.inputQueue = deck;
            this.controlCh.updateRegisters();
            return true;
        }
        return false;
    }
...
class BlackjackGameWithUi extends BlackjackGame {
    ...
    resetState() {
        if (!this.editMode) {
            document.getElementById('deck_button').disabled = false;
    ...
    nextLine() {
        // Once we've started the program we can no longer change the initial deck
        document.getElementById('deck_button').disabled = true;
    ...
    disableStepButtons(disabled) {
        const stepButtons = ['next_button', 'run_button', 'deck_button'].map(function(id) { return document.getElementById(id); });
